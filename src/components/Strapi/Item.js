		...
		<React.Fragment>
			<li>
				...
				>
					...
					<h1>
						{post.title} <FaArrowRight className="arrow" />
					</h1>
					<p className="meta">
						<span>
							<FaCalendar size={18} /> {post.created_at || "No Created Date"}
						</span>
						<span>
							<FaUser size={18} /> {(post && post.author && post.author.firstName) || "No Author"}{" "}
							{(post && post.author && post.author.lastName) || "No Author"}
						</span>
						<span>
							<FaTag size={18} />{" "}
							{(post && post.categories && post.categories.map(item => item.category).join(", ")) ||
								"No Categories"}
						</span>
					</p>
					<p
						dangerouslySetInnerHTML={{
							__html: post.ogDescription
						}}
					/>
				</Link>
			</li>
